...
const phonepeurl = process.env.PHONE_PE_HOST_URL;
const merchandi = process.env.MERCHANT_ID;
const saltidx = process.env.SALT_INDEX;
...
app.get("/pay", (req, res) => {
  ...
  const xVerify =
    sha256(base63EncodedPayload + payEndpoint + saltkey) + "###" + saltidx;
  const options = {
    method: "post",
    url: `${phonepeurl}${payEndpoint}`,
    headers: {
